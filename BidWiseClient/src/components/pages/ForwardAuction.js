...
function ForwardAuction(){
    ...
    const [auctionInfo, setAuctionInfo]= useState({
        id:2,
        itemname: "Macwood",
        itemdesc: "Dive into the rich sounds of Fleetwood Mac with this authentic, used copy of the iconic 'Rumours' on vinyl. Released in 1977,this album has become a cornerstone of classic rock, featuring hits that have left an indelible mark on music history.",
        condition: "This vintage vinyl record is in good condition, preserving the warmth and authenticity of the original sound. While the cover and sleeve may show some signs of wear, the vinyl itself has been well cared for, ensuring an immersive and nostalgic listening experience.",
        initialprice:400,
    ...
        try {
          const response = await axios.get(`http://localhost:8080/bidding/productdetails?productid=${pid}`);
...
<p className="pcss"> <strong className="colorblk">Item Description:</strong>{auctionInfo.itemdesc} </p>

<p className="pcss"><strong className="colorblk">Condition:</strong>{auctionInfo.itemdesc} </p>
